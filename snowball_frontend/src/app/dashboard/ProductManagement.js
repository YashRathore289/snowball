...
export default function ProductManagement({ cacheKey }) {
  ...
  const [formData, setFormData] = useState({ productid: '', productname: '', productprice: '' });
  ...
  const saveSortOrder = useCallback(async () => {
    try {
      ...
      const result = await postData('product/update-product-sort-order', { products: orderData });
      if (result?.status) {
        setHasUnsavedChanges(false);
  ...
  const handleAddClick = useCallback(() => {
    setFormData({ productid: '', productname: '', productprice: '' });
    setIsAddMode(true);
    setIsEditMode(false);
  ...
  const handleEditClick = useCallback((product) => {
    ...
    setFormData({ productid: product.productid, productname: product.productname, productprice: product.productprice });
    setIsEditMode(true);
    setIsAddMode(false);
    setIsModalOpen(true);
  }, []);
  ...
  }, [isAddMode, formData, fetchProducts, showToast, cacheKey]);
  ...
  }, [fetchProducts, showToast, cacheKey]);
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-black bg-opacity-50">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-hidden">
          <div className={`px-6 py-4 flex justify-between items-center ${isAddMode ? 'bg-gradient-to-r from-emerald-600 to-emerald-700' : 'bg-gradient-to-r from-blue-600 to-blue-700'}`}>
            <h2 className="text-xl font-semibold text-white">{isAddMode ? 'Add New Product' : 'Edit Product'}</h2>
          ...
          <div className="p-6">
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <input type="number" name="productprice" value={formData.productprice} onChange={handleInputChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {products.map((product, index) => (
                <tr key={product.productid} draggable
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <div className="flex gap-2">
                      <button onClick={(e) => { e.stopPropagation(); handleEditClick(product); }}
                      ...
                      <button onClick={(e) => { e.stopPropagation(); handleDelete(product.productid); }}
    ...
    <div>
      ...
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage all products - drag to reorder, then save</p>
        </div>
