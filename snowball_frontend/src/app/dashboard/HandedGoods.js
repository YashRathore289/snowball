...
export default function HandedGoodsManagement({ cacheKey }) {
  ...
  const handleSelectSalesman = useCallback((cardid, salesmanid, isEdit = false) => {
    ...
    updateCard(cardid, {
      ...
      cardDate: salesmanid ? date : undefined,
    }, isEdit);
  }, [salesmen, updateCard, showToast, date]);
  ...
  const getReturnValue = useCallback((card) => {
    if (!card.returnExpr || card.returnExpr.trim() === '') return 0;
    return evaluateExpression(card.returnExpr);
  }, []);
  ...
  const handleSave = useCallback(async (card, isEdit = false) => {
    if (!card.salesmanid) { showToast('Please select a salesman first'); return; }
      ...
      details: JSON.stringify({
        ...
        items: validRows.map(r => {
          if (r.isAllBig) {
            ...
            return { productid: 'allbig', productname: 'All Big', qty: multipliedValue, price: 1, total: multipliedValue, isAllBig: true, allBigExpr: r.allBigExpr };
          }
  ...
  const performDelete = useCallback(async (handedgoodsid) => {
    ...
    } catch (error) { console.error('Error deleting record:', error); showToast('Error deleting record'); }
  ...
  const renderCard = useCallback((card, isEdit = false) => {
    const cardEffectiveDate = card.cardDate || date;
      ...
      <div key={card.cardid} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
          <div className="bg-blue-50/50 border border-blue-200 rounded-lg p-3">
            ...
            <select value={card.salesmanid || ''} onChange={(e) => handleSelectSalesman(card.cardid, e.target.value, isEdit)} className={`${inputBase} w-full cursor-pointer`}>
              ...
              {availableSalesmen.map(s => (<option key={s.salesmanid} value={s.salesmanid}>{s.fullname}</option>))}
              {card.salesmanid && !salesmen.find(s => String(s.salesmanid) === String(card.salesmanid)) && (<option value={card.salesmanid} disabled>{card.salesmanName || 'Unavailable'} (Not Available)</option>)}
            </select>
          </div>
        ...
        {card.salesmanLocked && (
          <div className="space-y-3">
              ...
              return (
                <div key={row.rowid} className="flex items-center gap-2">
                  <select value={row.productid} onChange={(e) => handleSelectProduct(card.cardid, row.rowid, e.target.value, isEdit)} className={`${inputBase} flex-1 min-w-0 cursor-pointer`}>
                    ...
                    {products.map(p => (<option key={p.productid} value={p.productid}>{p.productname} (₹{parseFloat(p.productprice || 0).toFixed(2)})</option>))}
                  </select>
                  {row.isAllBig ? (
                    <>
                      <input type="text" placeholder="e.g. 10+23+45" value={row.allBigExpr} onChange={(e) => updateRow(card.cardid, row.rowid, { allBigExpr: e.target.value }, isEdit)} className={`${inputBase} flex-1 min-w-0`} />
                      <div className={`${inputBase} w-20 text-end bg-gray-50 text-gray-700`}>₹{((evaluateExpression(row.allBigExpr) || 0) * 10).toFixed(0)}</div>
                    </>
                  ) : (
                    <>
                      <input type="text" inputMode="numeric" placeholder="Qty" value={row.qty} onChange={(e) => updateRow(card.cardid, row.rowid, { qty: e.target.value }, isEdit)} className={`${inputBase} w-14 text-center`} />
            ...
            <div className="flex items-center justify-end gap-2 pt-1 border-t border-gray-100 pr-17"><span className="text-sm font-medium text-gray-700">Items Total:</span><div className={`${inputBase} w-20 text-end bg-gray-50 font-medium`}>₹{cardItemsTotal(card).toFixed(0)}</div></div>
            <div className="flex items-center justify-end gap-2 pr-17"><span className="text-sm font-medium text-gray-700">Return:</span><input type="text" placeholder="e.g. 40 + 512 + 8" value={card.returnExpr} onChange={(e) => updateCard(card.cardid, { returnExpr: e.target.value, saved: false }, isEdit)} className={`${inputBase} w-40`} /><div className={`${inputBase} w-20 text-end bg-gray-50 font-medium ${getReturnValue(card) === null ? 'text-red-500' : 'text-gray-700'}`}>₹{getReturnValue(card) === null ? '?' : getReturnValue(card).toFixed(0)}</div></div>
            <div className="flex items-center justify-end gap-2 pt-1 border-t border-gray-100 pr-17"><span className="text-sm font-medium text-gray-700">Total:</span><div className={`${inputBase} w-20 text-end bg-gray-50 font-medium`}>₹{(cardItemsTotal(card) - (getReturnValue(card) || 0)).toFixed(0)}</div></div>
            <div className="flex items-center justify-end gap-2 pr-17"><span className="text-sm font-medium text-gray-700">Commission:</span><input type="text" inputMode="numeric" placeholder="Commission" value={card.commission} onChange={(e) => updateCard(card.cardid, { commission: e.target.value, saved: false }, isEdit)} className={`${inputBase} w-20 text-end`} /></div>
            <div className="flex items-center justify-end gap-2 pt-1 border-t border-gray-200 pr-17">
              <label className="flex items-center gap-2 cursor-pointer"><span className="text-sm font-medium text-gray-700">Clear:</span><input type="checkbox" checked={card.clearStatus} onChange={(e) => updateCard(card.cardid, { clearStatus: e.target.checked, saved: false }, isEdit)} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" /></label>
              <div className="flex items-center gap-2"><span className="text-sm font-medium text-gray-700">Submit Amount:</span><input type="text" inputMode="numeric" placeholder="0" value={card.submitAmount} onChange={(e) => updateCard(card.cardid, { submitAmount: e.target.value, saved: false }, isEdit)} className={`${inputBase} w-24 text-end`} /></div>
              <span className="text-sm font-semibold text-gray-900">Final Amount:</span><div className={`${inputBase} w-24 text-end bg-blue-50 font-bold text-blue-700 text-base overflow-auto`}>{getFinalAmount(card) !== null ? `₹${getFinalAmount(card).toFixed(0)}` : <span className="text-gray-400 text-sm">-</span>}</div>
            </div>
            <div className="flex justify-end pt-2 pr-17">
              <button onClick={() => handleSave(card, isEdit)} disabled={card.saving} className={`px-6 py-2 rounded-md text-sm font-medium transition-colors cursor-pointer ${card.saved ? 'bg-green-100 text-green-700 border border-green-300 cursor-default' : (card.editMode || card.isUpdateMode || card.handedgoodsid) ? 'bg-yellow-600 hover:bg-yellow-700 text-white' : 'bg-indigo-600 text-white hover:bg-indigo-700'} ${card.saving ? 'opacity-60 cursor-not-allowed' : ''}`}>{card.saving ? 'Saving...' : card.saved ? '✓ Saved' : (card.editMode || card.isUpdateMode || card.handedgoodsid) ? 'Update Record' : 'Save Record'}</button>
              {card.saved && (<><button onClick={() => handleOpenBill(card)} className="ml-2 px-4 py-2 bg-gray-700 hover:bg-gray-800 text-white rounded-md text-sm font-medium transition-colors cursor-pointer">View Bill</button><button onClick={() => handleDownloadBill(card)} className="ml-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-md text-sm font-medium transition-colors cursor-pointer">Download Bill</button></>)}
              {card.editMode && card.saved && (<button onClick={() => { setEditCards([]); if (showRecords) fetchRecords(); }} className="ml-2 px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-md text-sm transition-colors cursor-pointer">Close</button>)}
            </div>
          </div>
        )}
      </div>
  ...
  const renderRecordsTable = useCallback(() => {
    ...
    const filteredRecords = recordSearch.trim()
      ? records.filter(r => r.salesman_name?.toLowerCase().includes(recordSearch.toLowerCase()) || String(r.handedgoodsid).includes(recordSearch))
      : records;
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">S.No</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">Salesman</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">Battery</th>
              <th className="px-4 py-3 text-right text-xs font-bold text-black uppercase tracking-wider">Item Total</th>
              <th className="px-4 py-3 text-right text-xs font-bold text-black uppercase tracking-wider">Return</th>
              <th className="px-4 py-3 text-right text-xs font-bold text-black uppercase tracking-wider">Commission</th>
              <th className="px-4 py-3 text-right text-xs font-bold text-black uppercase tracking-wider">Submit Amount</th>
              <th className="px-4 py-3 text-right text-xs font-bold text-black uppercase tracking-wider">Final Amount</th>
                ...
                <React.Fragment key={dateStr}>
                  ...
                  {groupedByDate[dateStr].map((record, idx) => {
                    ...
                    if (record.details?.items) {
                      record.details.items.forEach(item => {
                        totalItemAmount += parseFloat(item.total || 0) || 0;
                      });
                    }
                    return (
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      {toastVisible && (<div className="fixed top-4 right-4 z-50 bg-blue-600 text-white px-6 py-3 rounded-lg shadow-lg flex items-center gap-3"><span>{toastMessage}</span><button onClick={() => setToastVisible(false)} className="text-white hover:text-gray-200 font-bold text-lg leading-none cursor-pointer">×</button></div>)}
      {confirmVisible && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50"><div className="bg-white rounded-xl shadow-2xl p-6 w-96 max-w-[90%]"><h3 className="text-lg font-semibold text-gray-900 mb-4">Confirm Action</h3><p className="text-sm text-gray-600 mb-6">{confirmMessage}</p><div className="flex justify-end gap-3"><button onClick={() => setConfirmVisible(false)} className="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-lg transition-colors text-sm font-medium cursor-pointer">Cancel</button><button onClick={() => { if (confirmAction) confirmAction(); setConfirmVisible(false); }} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors text-sm font-medium cursor-pointer">Confirm</button></div></div></div>)}
      <div className="flex items-center justify-between mb-6">
        <div className={`${inputBase} flex items-center gap-2 font-medium`}><span className="text-black text-lg font-bold">Date:</span><input type="date" value={date} onChange={(e) => handleDateChange(e.target.value)} className="border-none outline-none text-lg font-bold bg-transparent cursor-pointer" /></div>
        <div className="flex gap-3">
          <button onClick={() => { setShowRecords(!showRecords); if (!showRecords) fetchRecords(); else setRecordSearch(''); }} className="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-md transition-colors text-sm font-medium cursor-pointer">{showRecords ? 'Hide Records' : 'View Records'}</button>
          <button onClick={addCard} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md transition-colors flex items-center gap-2 text-sm font-medium cursor-pointer"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>Add Salesman</button>
        </div>
      </div>
              ...
              <div className="flex items-center gap-3">
                ...
                {filterType === 'date' ? (<input type="date" value={date} onChange={(e) => { setDate(e.target.value) }} className="px-3 py-1.5 border border-gray-300 rounded-md text-sm cursor-pointer" />) : (
                  <div className="flex gap-2">
                    <select value={filterMonth} onChange={(e) => setFilterMonth(parseInt(e.target.value))} className="px-3 py-1.5 border border-gray-300 rounded-md text-sm cursor-pointer">{Array.from({ length: 12 }, (_, i) => i + 1).map(m => (<option key={m} value={m}>{new Date(2000, m - 1).toLocaleString('default', { month: 'long' })}</option>))}</select>
                    <select value={filterYear} onChange={(e) => setFilterYear(parseInt(e.target.value))} className="px-3 py-1.5 border border-gray-300 rounded-md text-sm cursor-pointer">{Array.from({ length: 5 }, (_, i) => new Date().getFullYear() - 2 + i).map(y => (<option key={y} value={y}>{y}</option>))}</select>
                  </div>
                )}
              </div>
            </div>
